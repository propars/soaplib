...
def join_attachment(href_id, envelope, payload, prefix=True):
    ...
    soapbody = soaptree.find("{%s}Body" % namespaces.ns_soap_env)

    message = None
    for child in list(soapbody):
        if child.tag != "{%s}Fault" % namespaces.ns_soap_env:
            message = child
            break
...
def apply_mtom(headers, envelope, params, paramvals):
    ...
    for i in range(len(params)):
        ...
        if typ == Attachment:
            ...
            incl = etree.SubElement(param, "{%s}Include" % namespaces.ns_xop)
